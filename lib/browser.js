...
class Browser {
  constructor(options = {}) {
    ...
    this.blockedHistory = []; // Engelleme geçmişi için dizi
    ...
    // User-Agent rotasyonu başlat (etkinleştirilmişse)
    this.userAgentManager = new UserAgentManager({
      enabled: options.userAgentRotation !== false,
      rotationStrategy: options.userAgentStrategy || 'random'
    });
  }
  ...
  // User-Agent rotasyonu ile ilgili yardımcı metodlar
  getUserAgentManager() {
    return this.userAgentManager;
  }
  
  setUserAgentRotation(enabled) {
    if (this.userAgentManager) {
      return this.userAgentManager.setEnabled(enabled);
    }
    return false;
  }
  
  setUserAgentStrategy(strategy) {
    if (this.userAgentManager) {
      return this.userAgentManager.setRotationStrategy(strategy);
    }
    return false;
  }
  ...
  getUserAgentStats() {
    if (this.userAgentManager) {
      return this.userAgentManager.getStats();
    }
    return { total: 0, enabled: false };
  }

  // setBlockDetectionSettings metodunu browser sınıfına ekle
  ...
  async setRandomUserAgent() {
    try {
      // User-Agent mevcut mu kontrol et
      if (!this.userAgentManager) {
        logger.warn('User-Agent Manager mevcut değil, varsayılan agent kullanılıyor');
      ...
      // UserAgentManager aktif mi kontrol et
      if (typeof this.userAgentManager.isEnabled !== 'function' || !this.userAgentManager.isEnabled()) {
        logger.debug('User-Agent rotasyonu devre dışı, varsayılan agent kullanılıyor');
        await this.page.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/100.0.4896.127 Safari/537.36');
        return;
      }
      ...
      let userAgent;
      if (typeof this.userAgentManager.getRandomUserAgent === 'function') {
        userAgent = this.userAgentManager.getRandomUserAgent();
      } else {
        // UserAgentManager'da fonksiyon yoksa, varsayılan UserAgent'lardan birisini kullan
        ...
        userAgent = defaultUserAgents[Math.floor(Math.random() * defaultUserAgents.length)];
        logger.warn('getRandomUserAgent fonksiyonu bulunamadı, varsayılan listeden alındı');
      }
      
      if (userAgent) {
        await this.page.setUserAgent(userAgent);
        logger.debug(`Rastgele User-Agent ayarlandı: ${userAgent}`);
      } else {
        logger.warn('Rastgele User-Agent alınamadı, varsayılan kullanılıyor');
        await this.page.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/100.0.4896.127 Safari/537.36');
      }
    } catch (error) {
