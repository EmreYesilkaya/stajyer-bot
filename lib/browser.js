...
class Browser {
  ...
  async launch() {
    try {
      ...
      // Tarayıcı argümanlarını hazırla - Incognito modunu burada etkinleştiriyoruz
      const defaultArgs = [
        '--incognito', // Incognito modunu etkinleştir
      ...
      // Headless modunu options'tan alıyoruz
      ...
      const launchOptions = {
        // Doğrudan false olarak ayarlıyoruz, argümanlar ile kontrol edeceğiz
        headless: false,
      ...
      // Mevcut sayfaları al (gizli mod olsa bile bu sayfalar oluşur)
      const pages = await this.browser.pages();
      logger.debug(`Başlangıç sayfa sayısı: ${pages.length}`);
      
      // İlk varsayılan sayfayı kullanacağız (bu sayfalar zaten incognito modda olacak)
      if (pages.length > 0) {
        this.page = pages[0];
        logger.debug('Varsayılan sayfa kullanılacak');
      } else {
        // Eğer hiç sayfa yoksa yeni bir sayfa aç
      ...
      // Hata durumunda alternatif başlatma yöntemini deneyelim
      try {
        logger.warn('Alternatif başlatma metodu deneniyor...');
        
        // Sandbox kısıtlamalarını kaldır ve temel argümanlarla dene
        const safeArgs = [
          '--incognito', // Yine incognito modunu ekle
          '--no-sandbox', 
        ...
        const executablePath = this.getChromeExecutablePath();
        if (executablePath) {
          altLaunchOptions.executablePath = executablePath;
        }
        
        logger.debug('Alternatif başlatma seçenekleri:', altLaunchOptions);
        
        this.browser = await puppeteer.launch(altLaunchOptions);
        
        // Mevcut sayfaları al
        const pages = await this.browser.pages();
        
        // İlk varsayılan sayfayı kullanacağız
        if (pages.length > 0) {
          this.page = pages[0];
        } else {
          // Eğer hiç sayfa yoksa yeni bir sayfa aç
        ...
        // Sayfa olaylarını kur
        this.setupPageEvents();
        
        logger.info('Tarayıcı alternatif modda başarıyla başlatıldı');
        return true;
      } catch (altError) {
        logger.error(`Alternatif başlatma da başarısız oldu: ${altError.message}`);
        return false;
      }
    }
  }
  ...
  async close() {
    ...
    // Önce sayfayı kapatmayı dene
    if (this.page) {
      try {
        await this.page.close();
        logger.debug('Sayfa başarıyla kapatıldı');
      } catch (pageErr) {
        logger.warn(`Sayfa kapatılırken hata oluştu: ${pageErr.message}`);
      } finally {
        this.page = null;
      }
    }
