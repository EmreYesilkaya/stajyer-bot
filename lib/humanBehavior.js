...
const randomScroll = async (page) => {
  try {
    ...
    logger.debug('Sayfa üzerinde gelişmiş scroll yapılıyor...');
    ...
    const totalHeight = Math.max(pageMetrics.bodyHeight, pageMetrics.documentHeight);
    const viewportHeight = pageMetrics.viewportHeight;
    
    if (totalHeight <= viewportHeight) {
      logger.debug('Sayfa kaydırılabilir değil (toplam yükseklik görünüm alanından küçük veya eşit)');
      return false;
    }
    
    // Scroll tipini rastgele seç
    const scrollTypes = ['smooth-down', 'smooth-up', 'random-jumps', 'read-like', 'fast-scan'];
    const scrollType = scrollTypes[Math.floor(Math.random() * scrollTypes.length)];
    
    logger.debug(`Seçilen scroll tipi: ${scrollType}`);
    
    switch (scrollType) {
      case 'smooth-down':
        // Yumuşak aşağı kaydırma - sayfa sonuna kadar yavaşça in
        await page.evaluate(() => {
          return new Promise((resolve) => {
            let totalHeight = 0;
            const distance = Math.floor(window.innerHeight * 0.7); // Her adımda ekranın %70'i kadar in
            const scrollDelay = Math.floor(Math.random() * (150 - 80)) + 80; // 80-150ms arası gecikme
      ...
      case 'smooth-up':
        // Önce aşağı sonra yumuşak yukarı kaydırma
        await page.evaluate(() => {
          // Önce sayfanın sonuna git
          window.scrollTo(0, document.body.scrollHeight);
        });
        await randomWait(800, 1500);
        
        // Sonra yukarı doğru yavaşça gel
        await page.evaluate(() => {
          return new Promise((resolve) => {
            let currentPosition = window.pageYOffset;
            const distance = Math.floor(window.innerHeight * 0.6); // Her adımda ekranın %60'ı kadar yukarı çık
            const scrollDelay = Math.floor(Math.random() * (130 - 70)) + 70; // 70-130ms arası gecikme
            
            const timer = setInterval(() => {
              currentPosition = Math.max(0, currentPosition - distance);
              window.scrollTo(0, currentPosition);
              
              // Sayfa başına ulaştık mı?
      ...
      case 'random-jumps':
        // Rastgele atlamalar - sayfada rastgele pozisyonlara atla
        const jumpCount = Math.floor(Math.random() * 4) + 3; // 3-6 arası atlama
        
        for (let i = 0; i < jumpCount; i++) {
          // Rastgele bir pozisyona scroll yap
          const position = Math.floor(Math.random() * totalHeight);
          ...
          // Rastgele bir süre bekle (800ms - 2000ms arası)
          await randomWait(800, 2000);
        }
        break;
        
      case 'read-like':
        // Okuma benzeri scroll - yavaş ve kademeli olarak ilerle
        await page.evaluate(() => {
          return new Promise((resolve) => {
            let totalHeight = 0;
            const smallDistance = Math.floor(window.innerHeight * 0.3); // Her adımda ekranın %30'u kadar in
            let variance = 0;
            
            // Scroll fonksiyonunu ayrı tanımla ki, setTimeout içinde kullanabilelim
            const scrollAction = () => {
              // Biraz değişkenlik ekle (-10px ile +10px arası)
              variance = Math.floor(Math.random() * 20) - 10;
              const adjustedDistance = smallDistance + variance;
              
              window.scrollBy(0, adjustedDistance);
              totalHeight += adjustedDistance;
              ...
              // Bazen biraz duraksama ekle (okuma gibi)
              if (Math.random() > 0.7) {
                clearInterval(scrollTimer);
                
                // Okuma duraksaması (1-3 saniye)
                setTimeout(() => {
                  if (totalHeight >= document.body.scrollHeight - window.innerHeight) {
                    resolve(); // Sayfa sonuna ulaştıysak bitir
                  } else {
                    // Yeni bir interval başlat
                    scrollTimer = setInterval(scrollAction, Math.floor(Math.random() * (200 - 100)) + 100);
                  }
            ...
            let scrollTimer = setInterval(scrollAction, Math.floor(Math.random() * (220 - 150)) + 150);
          });
        });
        break;
        
      case 'fast-scan':
        // Hızlı tarama - sayfayı hızlıca tara
        await page.evaluate(() => {
          return new Promise((resolve) => {
            let totalHeight = 0;
            const distance = Math.floor(window.innerHeight * 0.9); // Her adımda ekranın %90'ı kadar in
      ...
      default:
        // Varsayılan scroll davranışı (eski davranış)
        const scrollCount = Math.floor(Math.random() * 4) + 3; // 3-6 arası scroll
    ...
    // Sayfada son bir gezinti - bazen sayfa başına, bazen ortaya git
    const finalPosition = Math.random() > 0.5 ? 0 : Math.floor(totalHeight * 0.4);
    await page.evaluate((pos) => {
      window.scrollTo({
        top: pos,
        behavior: 'smooth'
